"""
CYBERGENOME AI 2.0 - Complete Multi-User RBAC SOC Platform
Default: admin / cybergenome2025
Public register = analyst only. Admin grants full access.
"""
...
try:
    ...
    def observe_once(learn_procs=True):
        return {"ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "features": {"cpu": 10, "mem": 40, "proc_count": 60, "conn_count": 15, "bytes_sent_delta": 0, "new_procs": 0}, "score": 5, "level": 1, "label": "NORMAL", "reasons": [], "ai_note": "Fallback"}
    def daily_summary():
...
try:
    ...
    def list_processes(limit=50):
        return [{"pid": 100, "name": "system.exe", "exe": "", "cpu": 5, "mem": 2, "risk": 1, "reasons": []}]
    def kill_process(pid): return {"ok": False, "msg": "Module missing"}
    def quarantine_file(path): return {"ok": False, "msg": "Module missing"}
...
try:
    ...
    def check_url_ioc(url): return {"matched": False, "source": None, "message": "IOC offline", "risk_boost": 0, "tags": []}
    def check_hash_ioc(h): return {"matched": False, "source": None, "message": "Hash intel offline", "risk_boost": 0}
    def refresh_urlhaus_recent(force=True): return {"ok": False, "count": 0}
    def intel_status(): return {"requests_installed": False, "feed_hosts": 0}

app = Flask(__name__)
app.secret_key = "cybergenome_rbac_secret_2025"
...
KW = ["login", "verify", "account", "password", "secure", "update", "bank", "confirm"]
EXT = [".exe", ".bat", ".cmd", ".ps1", ".vbs", ".js"]
...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS url_scans (
        ...
        mitre_id TEXT, scanned_by TEXT DEFAULT ''
    )""")
    cur.execute("""CREATE TABLE IF NOT EXISTS file_scans (
        ...
        risk_score INTEGER, risk_classification TEXT, mitre_id TEXT, scanned_by TEXT DEFAULT ''
    )""")
...
def get_windows_security_events():
    ...
    try:
        ...
        c.close()
        for r in rows:
            events.append({"id": f"CG-{r['event_type']}", "type": r["risk"] or "INFO",
                          "msg": f"{r['target'] or '-'} | {r['message'] or ''}", "time": r["timestamp"] or ""})
    except Exception:
        pass
    if not events:
        events = [{"id": "CG-SYSTEM", "type": "INFO", "msg": "SIEM online", "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}]
    return events
...
def ping_host(ip):
    ...
    try:
        r = subprocess.run(["ping", param, "1", "-w", "300", ip], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=1.0)
...
def scan_network_worker(subnet):
    ...
    my_ip = get_local_ip()
    discovered = [{"ip": my_ip, "hostname": "Host Workstation", "ports": [80, 443], "risk": 5 if my_ip in ISOLATED_DEVICES else 1, "status": "isolated" if my_ip in ISOLATED_DEVICES else "safe", "type": "LOCAL"}]
    results = {}
    def worker(ip):
        if ping_host(ip): results[ip] = True
    threads = []
    for i in range(2, 12):
        tip = f"{subnet}{i}"
        if tip == my_ip: continue
        t = threading.Thread(target=worker, args=(tip,))
        threads.append(t); t.start()
    for t in threads: t.join()
    for ip in results:
        discovered.append({"ip": ip, "hostname": "Subnet Endpoint", "ports": [80], "risk": 5 if ip in ISOLATED_DEVICES else 1, "status": "isolated" if ip in ISOLATED_DEVICES else "safe", "type": "LOCAL"})
...
def train_model():
    ...
    m.fit(X, y)
    joblib.dump(m, "models/threat_model.pkl")
    return m


def load_model():
    if not joblib: return None
    p = "models/threat_model.pkl"
...
if __name__ == "__main__":
    ...
    print("=" * 55)
    print("CYBERGENOME AI 2.0 - Multi-User RBAC Mode")
    print("Default Login: admin / cybergenome2025")
    print("Register creates ANALYST only")
    print("=" * 55)
